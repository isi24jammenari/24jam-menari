...
type RundownItem = {
  ...
  performance: {
    ...
    works: { title: string; duration: number }[];
  } | null;
};
...
export default function RundownTab() {
  ...
  const handleDownload = () => {
    const rows = rundownData.map((item) => {
      // Ambil semua judul karya dan gabungkan dengan koma
      const allWorks = item.performance?.works?.map(w => w.title).join(" | ") || "-";

      return {
        "Venue": item.timeSlot?.venue?.name || "-",
        "Jam Show": item.timeSlot?.time_range || "-",
        "Akun Pendaftar": item.user?.name || "-",
        "Email Pendaftar": item.user?.email || "-",
        "Nama Kelompok/Sanggar": item.performance?.group_name || "-",
        "Nama Narahubung": item.performance?.cp_name || "-",
        "WA Narahubung": item.performance?.contact_person || "-",
        "Judul Karya": allWorks,
  ...
  const getStatusBadge = (perf: RundownItem['performance']) => {
    if (!perf) return <Badge className="bg-destructive/10 text-destructive border-destructive/20 border">✕ Kosong</Badge>;
    if (perf.status === "completed") return <Badge className="bg-primary/10 text-primary border-primary/20 border">✓ Final</Badge>;
    return <Badge className="bg-accent/10 text-accent border-accent/20 border">✎ Draft</Badge>;
  };

  if (isLoading) return <div className="text-center py-10 animate-pulse text-muted-foreground">Memuat tabel rundown...</div>;

  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h3 className="text-tradisional text-2xl font-bold text-primary">Rundown & Database Peserta</h3>
          <p className="text-sm text-muted-foreground mt-0.5">Daftar seluruh transaksi yang berhasil dibayar.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted border-b border-border">
                ...
                <th className="text-left px-4 py-3 font-semibold text-muted-foreground whitespace-nowrap">Akun / Kontak</th>
                <th className="text-left px-4 py-3 font-semibold text-muted-foreground whitespace-nowrap">Nama Kelompok</th>
                <th className="text-left px-4 py-3 font-semibold text-muted-foreground whitespace-nowrap">Karya</th>
            ...
            <tbody className="divide-y divide-border">
              {rundownData.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-8 text-muted-foreground">Belum ada peserta.</td></tr>
              ) : (
                rundownData.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-foreground">{item.performance?.cp_name || item.user?.name}</p>
                    ...
                    <td className="px-4 py-3 text-foreground">
                      {item.performance?.works?.map(w => w.title).join(", ") || <span className="text-muted-foreground italic">—</span>}
                    </td>
                    <td className="px-4 py-3">{getStatusBadge(item.performance)}</td>
                  </tr>
                ))
              )}
            </tbody>
